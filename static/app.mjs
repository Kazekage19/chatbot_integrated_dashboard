...
function populateTable(marketType, year) {
  ...
  if (marketType !== "Combined") {
    filteredData = filteredData.filter(
      (entry) => entry["Market Type"] === marketType
    );
  }
  filteredData.sort(
    (a, b) =>
      b["Gross Written Premium (GWP)"] - a["Gross Written Premium (GWP)"]
  );
  tableBody.innerHTML = "";
  filteredData.slice(0, 10).forEach((entry) => {
    const row = document.createElement("tr");
    const difference = calculateDifference(
      entry["Gross Written Premium (GWP)"],
      entry["Planned GWP"]
    );
    row.innerHTML = `
...
function populateTable2(businessType, classType) {
  ...
  tableBody.innerHTML = "";
  const anotherData = jsonData.filter(
    (entry) => entry["Class of Business"] == businessType
  );
  const filteredData =
    classType === "All"
      ? anotherData
      : anotherData.filter((entry) => entry["ClassType"] === classType);
...
document
  .getElementById("business-type-select")
  .addEventListener("change", function () {
...
document
  .getElementById("class-type-select")
  .addEventListener("change", function () {
    const business_class = document.getElementById(
